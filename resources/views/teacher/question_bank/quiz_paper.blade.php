...
<head>
<meta charset="utf-8">
<title>{{ $quiz->title }} — {{ get_phrase('Exam paper') }}</title>
<style>
  * { box-sizing: border-box; font-family: 'DejaVu Sans', Arial, Helvetica, sans-serif; }
  ...
  .toolbar a, .toolbar button { background:#00955f; color:#fff; border:0; border-radius:8px;
  ...
  .head { text-align:center; border-bottom:3px solid #00955f; padding-bottom:14px; margin-bottom:8px; }
  .head img { width:74px; margin-bottom:6px; }
  .head .org { font-size:17px; font-weight:800; color:#00955f; letter-spacing:.4px; }
  ...
  .opt.correct .bullet { background:#00955f; border-color:#00955f; }
  .opt.correct { color:#00794c; font-weight:700; }
...
<div class="toolbar">
  <div style="font-weight:700;"><span style="opacity:.7;">{{ get_phrase('Exam paper preview') }} —</span> {{ $quiz->title }}</div>
  <div style="display:flex; gap:8px; flex-wrap:wrap;">
    <a class="ghost" href="{{ route('teacher.quiz.questions', $quiz->id) }}">&larr; {{ get_phrase('Questions') }}</a>
    @if($answers)
      <a class="ghost" href="{{ route('teacher.quiz.paper', $quiz->id) }}">{{ get_phrase('Hide answer key') }}</a>
    @else
      <a class="ghost" href="{{ route('teacher.quiz.paper', ['id' => $quiz->id, 'answers' => 1]) }}">{{ get_phrase('Show answer key') }}</a>
    @endif
...
<div class="sheet">
  <div class="head">
    ...
    <div class="doc">{{ $quiz->title }} @if($answers)<span style="color:#c0392b;">— {{ get_phrase('ANSWER KEY') }}</span>@endif</div>
  </div>

  <div class="meta">
    <span><b>{{ get_phrase('Class') }}:</b> {{ $className }} · {{ $sectionName }}</span>
    <span><b>{{ get_phrase('Subject') }}:</b> {{ $subjectName }}</span>
    <span><b>{{ get_phrase('Total marks') }}:</b> {{ $quiz->total_marks }}</span>
    @if($quiz->duration_minutes)<span><b>{{ get_phrase('Time') }}:</b> {{ $quiz->duration_minutes }} {{ get_phrase('minutes') }}</span>@endif
    @if($quiz->deadline)<span><b>{{ get_phrase('Date') }}:</b> {{ date('d M Y', (int) $quiz->deadline) }}</span>@endif
  </div>
  ...
  <div class="instr">
    <b>{{ get_phrase('Instructions:') }}</b>
    {{ $quiz->description ?: get_phrase('Answer ALL questions. Write clearly. For multiple-choice questions, shade one option only.') }}
  </div>
  ...
  @foreach($attached as $i => $aq)
    ...
    <div class="q">
      <div class="qt">
        ...
        <span class="mk">[{{ $aq->marks }} {{ get_phrase($aq->marks == 1 ? 'mark' : 'marks') }}]</span>
      </div>
      ...
      @elseif($q->type === 'short')
        @if($answers)
          <div class="key">{{ get_phrase('Expected answer') }}: {{ $q->correct_answer ?: get_phrase('(marked manually)') }}</div>
        @else
          <div class="lines"><div class="ln"></div><div class="ln"></div></div>
        @endif
      @else {{-- essay --}}
        @if($answers)
          <div class="key">{{ get_phrase('Essay — marked manually out of') }} {{ $aq->marks }}.</div>
        @else
          <div class="lines">@for($l = 0; $l < 6; $l++)<div class="ln"></div>@endfor</div>
        @endif
      @endif
    </div>
  @endforeach
  ...
  @if($attached->count() === 0)
    <p style="text-align:center;color:#9aa1b0;padding:40px 0;">{{ get_phrase('This CAT has no questions yet.') }}</p>
  @endif

  <div class="foot">
    {{ $school->title ?? '' }} — {{ $quiz->title }} · {{ $className }} {{ $sectionName }} · {{ get_phrase('END OF PAPER') }}
  </div>
</div>
