...
<div class="mainSection-title">
  <div class="row"><div class="col-12">
    <div class="d-flex flex-column"><h4>{{ get_phrase('Fees') }}</h4>
...
<div class="row"><div class="col-12"><div class="eSection-wrap">
  <div class="table-responsive">
    <table class="table eTable eTable-2">
      <thead><tr><th>{{ get_phrase('Child') }}</th><th>{{ get_phrase('Invoice') }}</th><th class="text-end">{{ get_phrase('Total') }}</th><th class="text-end">{{ get_phrase('Balance') }}</th><th>{{ get_phrase('Due') }}</th><th>{{ get_phrase('Status') }}</th></tr></thead>
      <tbody>
        @forelse($invoices as $inv)
          @php $child=\App\Models\User::find($inv->student_id); @endphp
          <tr>
            <td>{{ $child->name ?? '-' }}</td>
